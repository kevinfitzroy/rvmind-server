...
export class ModbusController {
    ...
    getSystemStatus(): ModbusSystemStatus {
        ...
        const totalErrors24h = systemStatus.queues.reduce((sum, queue) => sum + queue.errorCount24h, 0);
        const totalErrors1m = systemStatus.queues.reduce((sum, queue) => sum + queue.errorCount1m, 0);
        const totalAccesses1m = systemStatus.queues.reduce((sum, queue) => sum + queue.accessCount1m, 0);
