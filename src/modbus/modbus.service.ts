...
export class ModbusService implements OnModuleInit, OnModuleDestroy {
    ...
    private cleanupExpiredErrors(): void {
        ...
        for (const [port, stats] of this.errorStats.entries()) {
            ...
            if (cleaned24h > 0 || cleaned1m > 0) {
                console.log(`Port ${port} cleaned errors: 24h(${cleaned24h}), 1m(${cleaned1m})`);
            }
    ...
    private getErrorStats(port: ModbusPort): { count24h: number; count1m: number; lastError?: ErrorRecord } {
        const stats = this.errorStats.get(port);
        if (!stats) {
        ...
        const lastError = stats.errors24h.length > 0 ? 
            stats.errors24h[stats.errors24h.length - 1] : undefined;
    ...
    private getAccessStats(port: ModbusPort): { count1m: number; lastAccess?: AccessRecord } {
        const stats = this.accessStats.get(port);
        if (!stats) {
            return { count1m: 0 };
        }
        ...
        const lastAccess = stats.accesses1m.length > 0 ? 
            stats.accesses1m[stats.accesses1m.length - 1] : undefined;
    ...
    private getDeviceOfflineState(port: ModbusPort, address: number): DeviceOfflineState {
    ...
    private markDeviceOffline(port: ModbusPort, address: number): void {
        ...
        console.warn(`设备 ${port}:${address} 标记为离线，进入 ${state.cooldownDuration}ms 冷却期`);
    }
    ...
    async enqueueRequest<T>(
        ...
        if (this.isDeviceInCooldown(port, deviceAddress)) {
            ...
            const remainingCooldown = state.cooldownDuration - (Date.now() - state.lastFailedTime);
            throw new Error(`设备 ${port}:${deviceAddress} 处于离线冷却期，剩余 ${Math.ceil(remainingCooldown / 1000)} 秒`);
        }
        ...
        return new Promise<T>((resolve, reject) => {
            const queuedRequest: QueuedRequest = {
                ...
                task: async (client: ModbusRTU) => {
                    ...
                    console.log(`Processing request for port ${port} with device address ${deviceAddress}`);
                    try {
    ...
    private async processQueue(port: ModbusPort): Promise<void> {
        ...
        if (nextRequest) {
            try {
                ...
                console.error(`Error processing request for port ${port}, device ${nextRequest.deviceAddress}:`, error);
    ...
    getSystemStatus(): ModbusSystemStatus {
        ...
        const totalQueuedRequests = queues.reduce((sum, queue) => sum + queue.queueLength, 0);
        const activeProcessingPorts = queues.filter(queue => queue.isProcessing).length;

        const totalErrors24h = queues.reduce((sum, queue) => sum + queue.errorCount24h, 0);
        const totalErrors1m = queues.reduce((sum, queue) => sum + queue.errorCount1m, 0);
        ...
        return {
            ...
            totalErrors24h,
            totalErrors1m,
            totalAccesses1m: queues.reduce((sum, queue) => sum + queue.accessCount1m, 0)
        };
    }
