...
export class RelayController {
    ...
    getDevice(@Param('deviceId') deviceId: string) {
        try {
            return this.relayService.getDeviceConfigs().find(device => device.id === deviceId);
        } catch (error) {
            throw new HttpException(`Device not found: ${deviceId}`, HttpStatus.NOT_FOUND);
        }
    }
    ...
    async getRelayState(@Param('deviceId') deviceId: string) {
        try {
            ...
            throw new HttpException(`Failed to get relay state for device: ${deviceId}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async getInputState(@Param('deviceId') deviceId: string) {
        try {
            ...
            throw new HttpException(`Failed to get input state for device: ${deviceId}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async getDeviceOnlineStatus(@Param('deviceId') deviceId: string) {
        try {
            ...
            if (error.message.includes('冷却期')) {
                return { success: true, deviceId, isOnline: false, reason: '设备在冷却期' };
            }
    ...
    async turnButtonOn(@Param('buttonId') buttonId: string) {
        try {
            ...
            if (error.message.includes('冷却期')) {
                throw new HttpException(`设备暂时离线: ${error.message}`, HttpStatus.SERVICE_UNAVAILABLE);
            }
            throw new HttpException(`Failed to turn button on: ${buttonId}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async turnButtonOff(@Param('buttonId') buttonId: string) {
        try {
            ...
            throw new HttpException(`Failed to turn button off: ${buttonId}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    private async toggleRelay(deviceId: string, relayId: string, state: RELAY_STATE): Promise<{ success: true }> {
        const relayIndex = parseInt(relayId, 10);
        if (isNaN(relayIndex)) {
            throw new HttpException(`Invalid relay index: ${relayId}`, HttpStatus.BAD_REQUEST);
        }
    ...
    async turnRelayOn(@Param('deviceId') deviceId: string, @Param('relayId') relayId: string) {
        try {
            const result = await this.toggleRelay(deviceId, relayId, RELAY_STATE.ON);
            return result;
        } catch (error) {
            console.log(error)
            throw new HttpException(`Failed to turn relay on for device: ${deviceId}, relay: ${relayId}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    @Post('device/:deviceId/:relayId/off')
    async turnRelayOff(@Param('deviceId') deviceId: string, @Param('relayId') relayId: string) {
        try {
            const result = await this.toggleRelay(deviceId, relayId, RELAY_STATE.OFF);
            return result;
        } catch (error) {
            throw new HttpException(`Failed to turn relay off for device: ${deviceId}, relay: ${relayId}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
}
