...
export class RelayService {
  ...
  getRelayController(
    ...
    if (!this.relayControllers.has(key)) {
      ...
      if (!deviceConfig) {
        throw new Error(
          `can not find device config; relay addr:${relayOptions.relayAddr}, modbus port: ${port}`,
        );
      }
...
class RelayController {
  ...
  private async performPeriodicUpdate(): Promise<void> {
    ...
    try {
      ...
      // 检查状态变化
      const relayChanges = this.detectRelayStateChanges(
        previousRelayState,
        newRelayState,
      );
      const inputChanges = this.detectInputStateChanges(
        previousInputState,
        newInputState,
      );
      ...
      if (relayChanges.length > 0 || inputChanges.length > 0) {
        ...
        console.log(
          `状态变化检测 - 设备 ${this.deviceId}: 继电器变化 ${relayChanges.length} 个, 输入变化 ${inputChanges.length} 个`,
        );
      }
  ...
  private detectRelayStateChanges(
    oldStates: RELAY_STATE[],
    newStates: RELAY_STATE[],
  ): RelayStateChange[] {
  ...
  private detectInputStateChanges(
    oldStates: RELAY_STATE[],
    newStates: RELAY_STATE[],
  ): InputStateChange[] {
    ...
    try {
      const states = await this.modbusService.enqueueRequest<RELAY_STATE[]>(
        ...
        async (client) => {
          ...
          return data
            .slice(0, this.relayLength)
            .map((v) => (v ? RELAY_STATE.ON : RELAY_STATE.OFF));
        },
        highPriority,
      );
  ...
  async readInputState(highPriority = false): Promise<RELAY_STATE[]> {
    ...
    try {
      const states = await this.modbusService.enqueueRequest<RELAY_STATE[]>(
        ...
        this.relayAddr,
        async (client) => {
          const data = (await client.readDiscreteInputs(0, this.relayLength))
            .data;
          return data
            .slice(0, this.relayLength)
            .map((v) => (v ? RELAY_STATE.ON : RELAY_STATE.OFF));
