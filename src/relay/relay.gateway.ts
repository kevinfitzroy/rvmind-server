...
export class RelayGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleGetDeviceStatus(
        ...
        try {
            const controller = this.relayService.getRelayControllerFromConfig(data.deviceId);
    ...
    private async sendInitialStates(client: Socket) {
        try {
            ...
            for (const config of deviceConfigs) {
                try {
                    const controller = this.relayService.getRelayControllerFromConfig(config.id);
